...
func (curve *CurveParams) AddGeneric(x1, y1, x2, y2 *big.Int) (*big.Int, *big.Int) {
	...
	lamda.Sub(y2, y1)
	lamda.Div(lamda, new(big.Int).Sub(x2,x1))
...
func (curve *CurveParams) DoubleGeneric(x1, y1 *big.Int) (*big.Int, *big.Int) {
	fmt.Println(x1.String(), y1.String())
...
func (curve *CurveParams) ScalarMultGeneric(Bx, By *big.Int, k []byte) (*big.Int, *big.Int) {
	...
	for _, byte := range k {
		for bitNum := 0; bitNum < 8; bitNum++ {
			x, y = curve.DoubleGeneric(x, y)
			if byte&0x80 == 0x80 {
				x, y = curve.AddGeneric(Bx, By, x, y)
			}
			byte <<= 1
		}
	}
